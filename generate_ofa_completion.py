...
def takes_file(cmd: click.Command) -> bool:
    ...
    Args:
        cmd (click.Command): The command to check.

    Returns:
        bool: True if the command takes a file or folder argument, False otherwise.
    """
    for p in cmd.params:
...
def collect_flags(cmd: click.Command) -> list[str]:
    ...
    Returns:
        list[str]: A list of flags (short and long) for the command.
    """
    flags = []
    for p in cmd.params:
